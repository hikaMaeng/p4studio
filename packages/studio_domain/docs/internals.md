...
- [runner](../src/server/deployments/runner.ts)는 transport와 persist를 주입받는다. stage별 결과와 전송 불명을 I/O 구현과 분리해 검증하기 위한 경계다. 이 설계가 모든 실패를 이미 처리한다는 뜻은 아니다.

P4 engine/KV 내부 계산을 이 패키지로 가져오지 않는 이유와 원본 경로는 [P4 참조 안내](../../../docs/p4-reference.md)에 있다.

- 로딩은 노드별 독립 CREATE/LOAD다. SESSION과 추론 연결은 생성하지 않는다. 성공한 모든 노드의 Loaded와 llama.cpp build identity 합의가 있어야 전체 ready다.
...
- 현재 명령 전송은 한 ingress socket에서 순차 처리한다. UI GET은 2초 주기이며 native 세부 진행률을 만들어 내지 않는다.
