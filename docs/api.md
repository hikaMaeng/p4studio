...
| `POST` | `/api/agents` | SQLite 에이전트 등록 |
| `PATCH` | `/api/agents/:id` | SQLite 관리 이름·호스트·포트 수정 |
| `POST` | `/api/agents/:id/probe` | 브라우저 소유 조회 안내: 409 |
| `POST` | `/api/agents/:id/nodes` | 폐기된 단독 노드 선언: 410, 모델 배치로 안내 |
...
에이전트 등록과 수정 입력은 `name`, `host`, `port`만 받는다. Studio 노드 선언도 `name`만 받는 SQLite 관리 데이터이며 P4 에이전트 identity나 원격 CREATE가 아니다. 실행 어댑터는 모델 배치의 Agent-target LOAD에서 선택한다. 단독 노드 선언 POST는 410이며 과거 SQLite 데이터는 보존한다.
