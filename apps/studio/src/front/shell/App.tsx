...
export const App = () => {
  ...
  const view: View = route.kind === "overview" ? "overview" : route.kind.startsWith("agent") ? "agents" : route.kind.startsWith("model") ? "models" : route.kind.startsWith("inference") ? "inference" : "overview";
  const go = (next: StudioRoute) => navigate(next);
  const openPrimary = () => go(view === "models" ? { kind: "model-new" } : { kind: "agent-new" });
  const inferenceTab = route.kind === "inference-history-detail" ? "history" : route.kind === "inference" ? route.tab : "query";
  const inferenceRunId = route.kind === "inference-history-detail" ? route.runId : undefined;
  const inferenceRequestId = route.kind === "inference-request-detail" ? route.requestId : undefined;
  const content = snapshot && (route.kind === "overview" ? <DashboardView snapshot={snapshot} onRegister={() => go({ kind: "agent-new" })} /> : route.kind === "agent-groups" || route.kind === "agent-group-new" || route.kind === "agent-group-detail" || route.kind === "agent-group-edit" ? <AgentGroupsView groupId={route.kind === "agent-group-detail" || route.kind === "agent-group-edit" ? route.groupId : undefined} editor={route.kind === "agent-group-new" || route.kind === "agent-group-edit"} /> : view === "agents" ? <AgentsView snapshot={snapshot} selectedAgentId={route.kind === "agent-detail" || route.kind === "agent-node-new" ? route.agentId : undefined} selectedTab={route.kind === "agent-detail" ? route.tab : "nodes"} nodeCreation={route.kind === "agent-node-new"} onChanged={refresh} onRegister={() => go({ kind: "agent-new" })} onOpenAgent={(agentId) => go({ kind: "agent-detail", agentId, tab: "information" })} onBack={() => go({ kind: "agents" })} onSelectTab={(tab) => { if (route.kind === "agent-detail") go({ kind: "agent-detail", agentId: route.agentId, tab }); }} onDeclareNode={(agentId) => go({ kind: "agent-node-new", agentId })} onCloseNodeCreation={() => { if (route.kind === "agent-node-new") go({ kind: "agent-detail", agentId: route.agentId, tab: "nodes" }); }} /> : view === "models" ? <ModelsView snapshot={snapshot} selectedModelId={route.kind === "model-detail" || route.kind === "model-edit" ? route.modelId : undefined} editor={route.kind === "model-new" ? "new" : route.kind === "model-edit" ? "edit" : undefined} onOpenModel={(modelId) => go({ kind: "model-detail", modelId })} onCreate={() => go({ kind: "model-new" })} onEdit={(modelId) => go({ kind: "model-edit", modelId })} onCloseEditor={() => go(route.kind === "model-edit" ? { kind: "model-detail", modelId: route.modelId } : { kind: "models" })} onSaved={(modelId) => go({ kind: "model-detail", modelId })} /> : view === "inference" ? <InferenceView tab={inferenceTab} historyRunId={inferenceRunId} requestId={inferenceRequestId} onTab={(tab) => go({ kind: "inference", tab })} onOpenHistory={(runId) => go({ kind: "inference-history-detail", runId })} onBackHistory={() => go({ kind: "inference", tab: "history" })} onOpenRequest={(requestId) => go({ kind: "inference-request-detail", requestId })} onBackRequest={() => go({ kind: "inference", tab: "query" })} /> : null);
