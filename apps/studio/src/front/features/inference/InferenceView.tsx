...
import { RequestMonitoringContent, RunObservability } from "./InferenceObservability.js";
...
export function InferenceView({ tab, historyRunId, requestId, onTab, onOpenHistory, onBackHistory, onOpenRequest, onBackRequest }: { tab: "query" | "monitoring" | "history"; historyRunId?: string; requestId?: string; onTab: (tab: "query" | "monitoring" | "history") => void; onOpenHistory: (runId: string) => void; onBackHistory: () => void; onOpenRequest: (requestId: string) => void; onBackRequest: () => void }) {
  ...
  return <Box component="section" aria-label={t[RSC.INFERENCE_TITLE_TEXT]}>
    ...
    {requestId ? <RequestDetailPage requestId={requestId} onBack={onBackRequest} /> : tab === "query" ? <QueryPanel modelId={modelId} setModelId={setModelId} ready={ready} onOpenRequest={onOpenRequest} /> : tab === "monitoring" ? <MonitoringPanel modelId={modelId} ready={ready} /> : historyRunId ? <HistoryDetailPage runId={historyRunId} onBack={onBackHistory} /> : <HistoryListPage onOpen={onOpenHistory} />}
  </Box>;
}
function QueryPanel({ modelId, setModelId, ready, onOpenRequest }: { modelId: string; setModelId: (value: string) => void; ready: DeploymentRecord[]; onOpenRequest: (requestId: string) => void }) {
  ...
  const maxConcurrency = limits?.maxRequests ?? 1; const maxTokensPerRequest = limits?.maxOutputTokensPerRequest ?? 1;
  const maxRepetitions = maxInferenceRepetitions;
  useEffect(() => { setConcurrency(value => Math.min(value, maxConcurrency)); setRepetitions(value => Math.min(value, maxRepetitions)); setMaxTokens(value => Math.min(value, maxTokensPerRequest)); }, [maxConcurrency, maxRepetitions, maxTokensPerRequest]);
  ...
  return <Box sx={{ display: "grid", gap: 2.5, minWidth: 0 }}><Paper variant="outlined" sx={{ p: { xs: 2, md: 2.5 } }}><Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "2fr repeat(4, 1fr)" }, gap: 1.5 }}>
    ...
    <TextField type="number" label={t[RSC.INFERENCE_MAX_TOKENS_LABEL]} value={maxTokens} slotProps={{ htmlInput: { min: 1, max: maxTokensPerRequest } }} onChange={event => setMaxTokens(Math.min(maxTokensPerRequest, Math.max(1, Number(event.target.value))))} />
...
function RequestRow({ run, request, onOpen }: { run: InferenceRun; request: InferenceRequest; onOpen?: (requestId: string) => void }) { const { t } = useTranslation(); const state = request.state ?? "unknown", stateKey = runState[state] ?? RSC.INFERENCE_UNKNOWN_STATUS; return <TableRow data-testid="inference-request-row" sx={{ "& > *": { py: 1, whiteSpace: "nowrap" } }}><TableCell sx={resultCellSx}><Typography variant="body2" noWrap sx={{ maxWidth: 180 }}>{request.id}</Typography><Typography variant="caption" color="text.secondary" noWrap sx={{ display: "block", maxWidth: 180 }}>{run.modelName}</Typography></TableCell><TableCell sx={resultCellSx}><Chip size="small" label={t[stateKey]} color={state === "completed" ? "success" : ["failed", "unknown"].includes(state) ? "warning" : "default"} /></TableCell><TableCell sx={{ ...resultCellSx, maxWidth: 0 }}><Typography noWrap title={request.prompt || t[RSC.INFERENCE_VALUE_UNAVAILABLE_TEXT]}>{request.prompt || t[RSC.INFERENCE_VALUE_UNAVAILABLE_TEXT]}</Typography></TableCell><TableCell align="right" sx={metricCellSx}>{request.receivedTokens}</TableCell><TableCell align="right" sx={metricCellSx}>{metric(request.finalTps)}</TableCell>{onOpen && <TableCell align="right" sx={metricCellSx}><Button size="small" onClick={() => onOpen(request.id)}>{t[RSC.INFERENCE_HISTORY_OPEN_BUTTON]}</Button></TableCell>}</TableRow>; }
