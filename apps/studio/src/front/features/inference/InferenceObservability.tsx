...
export function RequestMonitoringContent({ run, request }: { run: InferenceRun; request: InferenceRequest }) {
  ...
  return <Box sx={{ display: "grid", gap: 2 }}>
    ...
    <Box sx={{ display: "grid", gridTemplateColumns: { xs: "repeat(2, 1fr)", md: "repeat(4, 1fr)" }, gap: 1.5 }}>{[
      [t[RSC.INFERENCE_TTFT_LABEL], fmt(request.ttftMs, " ms")], [t[RSC.INFERENCE_REQUEST_MONITORING_E2E_LABEL], fmt(e2e, " ms")], [t[RSC.INFERENCE_GENERATION_TPS_LABEL], fmt(request.generationTps)], [t[RSC.INFERENCE_REQUEST_MONITORING_BATCH_FILL_LABEL], fmt(fill === null ? null : fill * 100, "%")],
      [t[RSC.INFERENCE_REQUEST_MONITORING_ISSUES_LABEL], telemetry.issueCount], [t[RSC.INFERENCE_REQUEST_MONITORING_MIXED_LABEL], telemetry.mixedPhysicalBatches], [t[RSC.INFERENCE_REQUEST_MONITORING_PREFILL_ROWS_LABEL], telemetry.prefillRows], [t[RSC.INFERENCE_REQUEST_MONITORING_DECODE_ROWS_LABEL], telemetry.decodeRows],
