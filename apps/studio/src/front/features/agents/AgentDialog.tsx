...
export const AgentDialog = ({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated: (id: string) => void | Promise<void> }) => {
  ...
  const submit = async (event: React.FormEvent) => { event.preventDefault(); setBusy(true); setError(null); try { const agent = await studioApi.createAgent({ name, host, port: Number(port) }); setName(""); setHost(""); await onCreated(agent.id); } catch { setError(t[RSC.AGENTS_REGISTER_FAILURE_ALERT]); } finally { setBusy(false); } };
