...
const codeStyle = { fontSize: 13, lineHeight: 1.7 };
export function PlacementEditor({ id, index, snapshot }: { id: string; index: number; snapshot: StudioSnapshot }) {
  ...
  return <Paper component="fieldset" variant="outlined" sx={{ p: { xs: 2, md: 2.5 }, m: 0, minWidth: 0 }}>
    <Box component="legend" sx={{ px: .5 }}>{formatMessage(t[RSC.MODELS_STAGE_TEXT], { index: index + 1 })}</Box>
    <Box sx={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", mb: 2, gap: 1 }}><Typography variant="h2">{t[RSC.MODELS_STAGE_ROUTING_TEXT]}</Typography><Box><Button size="small" disabled={index === 0} onClick={() => deployments.moveStage(id, -1)}>{t[RSC.MODELS_MOVE_UP_BUTTON]}</Button><Button size="small" disabled={index === input.stages.length - 1} onClick={() => deployments.moveStage(id, 1)}>{t[RSC.MODELS_MOVE_DOWN_BUTTON]}</Button><Button size="small" color="secondary" onClick={() => deployments.removeStage(id)}>{t[RSC.MODELS_REMOVE_NODE_BUTTON]}</Button></Box></Box>
    <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1.5fr 2fr 1fr" }, gap: 1.5 }}>
      ...
      <TextField label={t[RSC.MODELS_NODE_LABEL]} value={stage.nodeId} slotProps={{ input: { readOnly: true } }} />
    ...
    {input.adapter === "llamacpp" ? <Box sx={{ display: "grid", gap: 2, mt: 2 }}>
      ...
      <TextField fullWidth multiline minRows={10} maxRows={28} label={t[RSC.MODELS_PLAN_LABEL]} value={stage.planText ?? ""} onChange={e => update("planText", e.target.value)} slotProps={{ input: { sx: codeStyle }, htmlInput: { spellCheck: false, dir: "ltr" } }} />
      ...
      <TextField fullWidth multiline minRows={18} maxRows={35} label={t[RSC.MODELS_LOAD_OPTIONS_LABEL]} value={stage.loadOptionsJson ?? "{}"} onChange={e => update("loadOptionsJson", e.target.value)} helperText={t[RSC.MODELS_LOAD_OPTIONS_MESSAGE]} slotProps={{ input: { sx: codeStyle }, htmlInput: { spellCheck: false, dir: "ltr" } }} />
    </Box> : <Box sx={{ display: "grid", gap: 2, mt: 2 }}><Box sx={{ display: "flex", gap: 2 }}><TextField type="number" label={t[RSC.MODELS_LAYER_START_LABEL]} value={stage.layerStart} onChange={e => update("layerStart", Number(e.target.value))} /><TextField type="number" label={t[RSC.MODELS_LAYER_END_LABEL]} value={stage.layerEnd} onChange={e => update("layerEnd", Number(e.target.value))} /></Box><TextField fullWidth multiline minRows={24} label={t[RSC.MODELS_PAYLOAD_LABEL]} value={stage.customPayload} onChange={e => update("customPayload", e.target.value)} slotProps={{ input: { sx: codeStyle }, htmlInput: { spellCheck: false, dir: "ltr" } }} /></Box>}
