...
export function NodeInspector({ snapshot }: { snapshot: StudioSnapshot }) {
  ...
  const fields = [[t[RSC.MODELS_AGENT_LABEL], agent?.name ?? selected.agentId], [t[RSC.MODELS_NODE_LABEL], selected.nodeId], [t[RSC.MODELS_GENERATION_LABEL], String(selected.nodeGeneration)], [t[RSC.MODELS_ADAPTER_LABEL], selected.adapterKind]];
